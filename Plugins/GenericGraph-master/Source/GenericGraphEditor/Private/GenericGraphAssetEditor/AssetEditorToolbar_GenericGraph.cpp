...
void FAssetEditorToolbar_GenericGraph::FillGenericGraphToolbar(FToolBarBuilder& ToolbarBuilder)
{
	...
	ToolbarBuilder.BeginSection("Util");
	{
		ToolbarBuilder.AddToolBarButton(FEditorCommands_GenericGraph::Get().AddGraphTemplate,
			NAME_None,
			LOCTEXT("AddGraphTemplate_Label", "Add Graph Template"),
			LOCTEXT("AddGraphTemplate_ToolTip", "Add a set of nodes to the graph in a pre determined template of your choice."),
			FSlateIcon(FGenericGraphEditorStyle::GetStyleSetName(), "GenericGraphEditor.AddGraphTemplate"));
	}
	ToolbarBuilder.EndSection();

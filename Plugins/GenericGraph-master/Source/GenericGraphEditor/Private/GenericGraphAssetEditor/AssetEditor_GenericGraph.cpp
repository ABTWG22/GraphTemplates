...
void FAssetEditor_GenericGraph::BindCommands()
{
	...
	ToolkitCommands->MapAction(FEditorCommands_GenericGraph::Get().AddGraphTemplate,
		FExecuteAction::CreateSP(this, &FAssetEditor_GenericGraph::AddGraphTemplate),
		FCanExecuteAction::CreateSP(this, &FAssetEditor_GenericGraph::CanAddGraphTemplate)
	);
}
...
void FAssetEditor_GenericGraph::CreateCommandList()
{
	...
	GraphEditorCommands->MapAction(FEditorCommands_GenericGraph::Get().AddGraphTemplate,
		FExecuteAction::CreateRaw(this, &FAssetEditor_GenericGraph::AddGraphTemplate),
		FCanExecuteAction::CreateRaw(this, &FAssetEditor_GenericGraph::CanAddGraphTemplate));
...
void FAssetEditor_GenericGraph::AddGraphTemplate()
{
	UEdGraph_GenericGraph* EdGraph = Cast<UEdGraph_GenericGraph>(EditingGraph->EdGraph);
	check(EdGraph != nullptr);

	const FScopedTransaction Transaction(LOCTEXT("GenericGraphEditorAddGraphTemplate", "Generic Graph Editor: Add Graph Template"));

	UGraphTemplate* GraphTemplate = nullptr;
	switch (GenricGraphEditorSettings->TemplateOption)
	{
	case ETemplateOption::Simple:
		GraphTemplate = NewObject<UGraphTemplate>(EdGraph, USimpleGraphTemplate::StaticClass());
		break;
	case ETemplateOption::Triangle:
		break;
	default:
		break;
	}

	if(GraphTemplate != nullptr)
	{
		GraphTemplate->Settings = GenricGraphEditorSettings;
		GraphTemplate->ConstructTemplate(EdGraph, this);
		GraphTemplate->ConditionalBeginDestroy();
	}
}

bool FAssetEditor_GenericGraph::CanAddGraphTemplate() const
{
	return true;
}
...
UEdGraphNode* FAssetEditor_GenericGraph::AddNewNodeToGraph(const FString& DesiredNodeTitle, UEdGraph* Graph, UEdGraphPin* FromPin, const FVector2D Location, bool bSelectNewNode) const
{
	const TSharedPtr<FAssetSchemaAction_GenericGraph_NewNode> NewNodeActionOne(new FAssetSchemaAction_GenericGraph_NewNode(LOCTEXT("GenericGraphNodeAction", "Generic Graph Node"), FText(), FText(), 0));
	NewNodeActionOne->NodeTemplate = NewObject<UEdNode_GenericGraphNode>(Graph);
	NewNodeActionOne->NodeTemplate->GenericGraphNode = NewObject<UGenericGraphNode>(NewNodeActionOne->NodeTemplate, EditingGraph->NodeType);
	NewNodeActionOne->NodeTemplate->GenericGraphNode->Graph = EditingGraph;
	NewNodeActionOne->NodeTemplate->GenericGraphNode->SetNodeTitle(FText::FromString(DesiredNodeTitle));

	return NewNodeActionOne->PerformAction(Graph, FromPin, Location, bSelectNewNode);
}
